...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        action='store_true',
        help='Only save checkpoint.pth during training to reduce disk usage.'
    )
...
def main(args):
    ...
    if args.frozen_weights is not None:
        checkpoint = torch_load_trusted(args.frozen_weights, map_location='cpu')
    ...
    if args.resume:
        if args.resume.startswith('https'):
            ...
            checkpoint = torch_load_trusted(args.resume, map_location='cpu')
    ...
    if (not args.resume) and args.pretrain_model_path:
        checkpoint = torch_load_trusted(args.pretrain_model_path, map_location='cpu')['model']
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            if (
                not args.keep_only_latest_checkpoint
                and ((epoch + 1) % args.lr_drop == 0 or (epoch + 1) % args.save_checkpoint_interval == 0)
            ):
        ...
        if _isbest and not args.keep_only_latest_checkpoint:
