...
class DINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        hs_tensor = torch.stack(hs) if isinstance(hs, list) else hs
        reference_tensor = torch.stack(reference) if isinstance(reference, list) else reference
        # In case num object=0
        hs_tensor[0] += self.label_enc.weight[0,0]*0.0

        # deformable-detr-like anchor update
        # reference_before_sigmoid = inverse_sigmoid(reference[:-1]) # n_dec, bs, nq, 4
        outputs_coord_list = []
        for dec_lid, (layer_ref_sig, layer_bbox_embed, layer_hs) in enumerate(zip(reference_tensor[:-1], self.bbox_embed, hs_tensor)):
        ...
        decoder_features_all = hs_tensor
        dn_decoder_features = None
        dn_aux_decoder_features = None
        if self.dn_number > 0 and dn_meta is not None and dn_meta.get('pad_size', 0) > 0:
            pad_size = dn_meta['pad_size']
            dn_decoder_features = hs_tensor[-1, :, :pad_size, :]
            dn_aux_decoder_features = hs_tensor[:-1, :, :pad_size, :]
            decoder_features_all = hs_tensor[:, :, pad_size:, :]

        outputs_class = torch.stack([layer_cls_embed(layer_hs) for
                                     layer_cls_embed, layer_hs in zip(self.class_embed, hs_tensor)])
        ...
        out = {
            ...
            'decoder_features': decoder_features_all[-1],
        }
        if decoder_features_all.shape[0] > 1:
            out['aux_decoder_features'] = decoder_features_all[:-1]
        if dn_decoder_features is not None:
            out['dn_decoder_features'] = dn_decoder_features
        if dn_aux_decoder_features is not None:
            out['dn_aux_decoder_features'] = dn_aux_decoder_features
        if self.aux_loss:
