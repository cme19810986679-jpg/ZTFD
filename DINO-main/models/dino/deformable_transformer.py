...
def compute_prototype_topk_scores(memory: Tensor, prototypes: Tensor, temperature: float = 0.2) -> Tensor:
    if memory.dim() != 3:
        raise ValueError("memory must be [B, N, D]")
    if prototypes.dim() != 3:
        raise ValueError("prototypes must be [C, R, D]")
    if memory.shape[-1] != prototypes.shape[-1]:
        raise ValueError("prototype dim must match memory dim")
    memory_norm = torch.nn.functional.normalize(memory, dim=-1)
    prototype_norm = torch.nn.functional.normalize(prototypes.reshape(-1, prototypes.shape[-1]), dim=-1)
    scores = torch.einsum("bnd,kd->bnk", memory_norm, prototype_norm) / temperature
    return scores.amax(dim=-1).clamp(-2.0, 2.0)


def compute_prototype_guided_query_selection(memory: Tensor, prototypes: Tensor, temperature: float = 0.2):
    """Return Zhang-style prototype target scores and selected class ids for encoder tokens."""
    if memory.dim() != 3:
        raise ValueError("memory must be [B, N, D]")
    if prototypes.dim() == 3:
        prototypes = prototypes.mean(dim=1)
    if prototypes.dim() != 2:
        raise ValueError("prototypes must be [C, D] or [C, R, D]")
    if memory.shape[-1] != prototypes.shape[-1]:
        raise ValueError("prototype dim must match memory dim")

    memory_norm = torch.nn.functional.normalize(memory, dim=-1)
    prototype_norm = torch.nn.functional.normalize(prototypes, dim=-1)
    attention_logits = torch.einsum("bnd,cd->bnc", memory_norm, prototype_norm) / temperature
    prototype_context = torch.softmax(attention_logits, dim=-1) @ prototype_norm
    class_scores = torch.einsum("bnd,cd->bnc", prototype_context, prototype_norm) / temperature
    target_scores, target_class_ids = class_scores.max(dim=-1)
    return target_scores.clamp(-2.0, 2.0), target_class_ids


def gather_prototype_content(prototypes: Tensor, class_ids: Tensor) -> Tensor:
    if prototypes.dim() == 3:
        prototypes = prototypes.mean(dim=1)
    if prototypes.dim() != 2:
        raise ValueError("prototypes must be [C, D] or [C, R, D]")
    flat_class_ids = class_ids.reshape(-1).clamp(0, prototypes.shape[0] - 1)
    gathered = prototypes.index_select(0, flat_class_ids)
    return gathered.reshape(*class_ids.shape, prototypes.shape[-1])
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.prototype_topk_prototypes = None
        self.prototype_topk_weight = None
        self.prototype_query_selection_prototypes = None
        self.prototype_query_selection_weight = None
        self.prototype_content_query_prototypes = None
        self.prototype_content_query_weight = None
        assert query_dim == 4
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type =='standard':
            ...
            topk_scores = enc_outputs_class_unselected.max(-1)[0]
            prototype_target_class_ids = None
            if (
                self.prototype_query_selection_prototypes is not None
                and self.prototype_query_selection_weight is not None
            ):
                prototype_target_scores, prototype_target_class_ids = compute_prototype_guided_query_selection(
                    output_memory,
                    self.prototype_query_selection_prototypes,
                )
                topk_scores = topk_scores + self.prototype_query_selection_weight * prototype_target_scores
            elif self.prototype_topk_prototypes is not None and self.prototype_topk_weight is not None:
                prototype_topk_scores = compute_prototype_topk_scores(
                    output_memory,
                    self.prototype_topk_prototypes,
                )
                topk_scores = topk_scores + self.prototype_topk_weight * prototype_topk_scores
            topk_proposals = torch.topk(topk_scores, topk, dim=1)[1] # bs, nq
            selected_prototype_class_ids = None
            if prototype_target_class_ids is not None:
                selected_prototype_class_ids = torch.gather(prototype_target_class_ids, 1, topk_proposals)
            ...
            if (
                selected_prototype_class_ids is not None
                and self.prototype_content_query_prototypes is not None
                and self.prototype_content_query_weight is not None
            ):
                selected_content = gather_prototype_content(
                    self.prototype_content_query_prototypes,
                    selected_prototype_class_ids,
                )
                tgt_ = tgt_ + self.prototype_content_query_weight * selected_content
